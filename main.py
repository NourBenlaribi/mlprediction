...
df = clean_data(df)
df.to_csv('./data/data_2014.csv')
df = pd.read_csv('./data/data_2014.csv')
...
plt.title("Precision with deleting row with at least nan value")
...
plt.title("Recall with deleting row with at least nan value")
plt.xlabel('Time window lenght(days)')
plt.ylabel('Recall')
plt.xticks([i for i in range(1,21)])
plt.yticks([i*0.1 for i in range(1,10)])
